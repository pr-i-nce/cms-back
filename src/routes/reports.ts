...
reportsRouter.get(
  ...
  async (_req, res) => {
    const data = await cached("reports:summary", env.cacheTtl.reportsMs, async () => {
      ...
      >`select substring(date_joined,1,7) as month, count(*)::int as count from members where date_joined is not null group by month order by month`;
      ...
      const deptCounts = await prisma.$queryRaw<
        { department_id: string; count: number }[]
      >`select department_id, count(*)::int as count from department_members group by department_id`;
      ...
      >`select substring(date,1,7) as month, status, count(*)::int as count from sms_records where date is not null group by month, status order by month`;

      const deliveredByMonth: Record<string, number> = {};
...
reportsRouter.get(
  ...
  async (_req, res) => {
    const data = await cached("reports:dashboard", env.cacheTtl.reportsMs, async () => {
      ...
      const latestMonthRow = await prisma.$queryRaw<{ month: string }[]>`select max(substring(date_joined,1,7)) as month from members`;
      ...
      >`select status, count(*)::int as count from sms_records group by status`;
