...
const getLoginKey = (email: string, req: any) => {
...
authRouter.post("/refresh", async (req, res) => {
  ...
  const refreshToken = parsed.data.refreshToken || (req as any).cookies?.refresh_token || "";
  if (!refreshToken) {
  ...
  try {
    ...
    const rows = await prisma.$queryRaw<{ user_id: string; expires_at: string; revoked_at: string | null }[]>`
