...
branchesRouter.get(
  ...
  async (req, res) => {
    ...
    const totalResult = await prisma.$queryRawUnsafe<{ count: bigint }[]>(
      `select count(*)::bigint as count from branches ${whereSql}`,
      ...params,
    );
...
branchesRouter.get(
  ...
  async (req, res) => {
    ...
    const pastorRows = await prisma.$queryRaw<any[]>`
...
branchesRouter.post(
  "/:id/deactivate",
  requireAuth,
  requirePermission("BRANCH_DEACTIVATE"),
  async (req, res) => {
    const actor = (req as any).userId || "system";
    const id = req.params.id;
    const now = new Date().toISOString();
...
branchesRouter.post(
  "/:id/pastors",
  requireAuth,
  requirePermission("BRANCH_PASTOR_ADD"),
  async (req, res) => {
    const id = req.params.id;
