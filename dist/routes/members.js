...
const findDuplicateMember = async (email, phone, excludeId) => {
    const or = [];
...
membersRouter.get("/", requireAuth, requirePermission("READ_ALL"), async (req, res) => {
...
membersRouter.get("/lookup", requireAuth, requireAnyPermission(["DEPT_MEMBER_ADD", "COMMITTEE_MEMBER_ADD"]), async (req, res) => {
...
membersRouter.post("/", requireAuth, requirePermission("MEMBER_CREATE"), async (req, res) => {
...
membersRouter.get("/pastors", requireAuth, requirePermission("READ_ALL"), async (_req, res) => {
...
membersRouter.get("/:id", requireAuth, requirePermission("READ_ALL"), async (req, res) => {
...
membersRouter.put("/:id", requireAuth, requirePermission("MEMBER_UPDATE"), async (req, res) => {
    const actor = req.userId || "system";
    const id = req.params.id;
...
membersRouter.delete("/:id", requireAuth, requirePermission("MEMBER_DELETE"), async (req, res) => {
    const actor = req.userId || "system";
    const id = req.params.id;
    const existing = await prisma.member.findUnique({ where: { id } });
    if (!existing) {
