...
const validateUuidParam = (paramName) => (value, res, next) => {
...
app.post("/api/metrics/ui-latency", requireAuth, async (req, res) => {
    ...
    const values = parsed.data.items.map((item) => Prisma.sql `('ui', 'NAV', ${item.path}, 200, ${item.durationMs}, ${item.timestamp})`);
    if (!values.length) {
        res.json(ok({ stored: 0 }));
        return;
    }
    await prisma.$executeRaw(Prisma.sql `insert into latency_logs (source, method, path, status, duration_ms, timestamp) values ${Prisma.join(values)}`);
